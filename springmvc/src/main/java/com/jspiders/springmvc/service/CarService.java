...
public class CarService {
	...
	public List<CarDTO> findAllCarsByUser(UserDTO signedInUser) {
		List<CarDTO> cars = carDAO.findAllCarByUser(signedInUser.getId());
		if (cars != null && cars.size() > 0) {
			return cars;
		} else {
			return null;
	...
	public List<CarDTO> findAllCarByPrice(double low, double high) {
		  List<CarDTO> cars = carDAO.findAllCarByPrice(low,high);
		  if (cars != null && cars.size() > 0) {
			return cars;
		}else {
		return null;
